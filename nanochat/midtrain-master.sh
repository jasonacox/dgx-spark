...
# This script runs distributed midtraining (fine-tuning) for the Nanochat model on the MASTER node
# in a two-node DGX Spark cluster connected via InfiniBand.
# This script should be run on the master node (NODE_RANK=0).
#
# Usage:
#   ./midtrain-master.sh [worker_host] [options]
#   
# Examples:
#   ./midtrain-master.sh spark-node02
#   ./midtrain-master.sh 192.168.100.5 --depth 24
#   ./midtrain-master.sh spark-worker --depth 16 --batch-size 16
#
# Credit: 
...
show_usage() {
    ...
    echo ""
    echo "Run distributed midtraining (fine-tuning) on MASTER node of a two-node DGX Spark cluster."
    echo ""
    echo "Arguments:"
    ...
    echo "  4. SSH key authentication must be set up"
    echo "  5. Run midtrain-worker.sh on the worker node AFTER starting this script"
    exit 1
}
...
print_status "DGX Spark Distributed Midtraining - MASTER NODE"
print_status "================================================"
echo ""
...
print_status "  ./midtrain-worker.sh $MASTER_ADDR --depth $DEPTH --batch-size $DEVICE_BATCH_SIZE --port $MASTER_PORT"
echo ""
...
for i in {10..1}; do
    echo -n -e "\r${BLUE}[MASTER]${NC} Starting midtraining in $i seconds... "
    sleep 1
done
echo ""
...
if [ $? -eq 0 ]; then
    ...
    print_status "  1. Test your model: source .venv/bin/activate && python -m scripts.chat_web"
    print_status "  2. Or run a chat CLI: ./chat.sh"
else
